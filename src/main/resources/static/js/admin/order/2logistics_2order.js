document.addEventListener("DOMContentLoaded", () => {
  ...
  const addModalTotalAmountEl = document.getElementById(
    "add-modal-total-amount",
  );
  ...
  navButtons.forEach((button) => {
    button.addEventListener("click", () => {
      ...
      const isSameOpenMenu =
        openedMenu === target && appShell.classList.contains("depth-open");
  ...
  document
    ...
    ?.addEventListener("click", async () => {
      ...
      const centerCode = document.getElementById(
        "modal-invoice-center-code",
      ).value;
      const writerName = document
        .getElementById("modal-invoice-writer")
        .value.trim();
      ...
      const items = Array.from(
        itemTableBody.querySelectorAll(".modal-item-row"),
      )
        .map((row) => ({
  ...
  const addRegularItemTableBody = document.getElementById(
    "add-regular-item-table-body",
  );
  const addRegularAddItemBtn = document.getElementById(
    "add-regular-add-item-btn",
  );
  ...
  function createRegularItemRow() {
    const tr = document.createElement("tr");
    tr.className = "modal-item-row";
    const classOptionsHtml = buildOptionsHtml(
      regularOptionsCache.classCodes,
      "classKey",
      "className",
      "품명 선택",
    );
    const userOptionsHtml = buildOptionsHtml(
      regularOptionsCache.users,
      "userCode",
      "userName",
      "선생님 선택",
    );
  ...
  function calculateRegularTotalAmount() {
    let total = 0;
    addRegularItemTableBody
      .querySelectorAll(".modal-item-row")
      .forEach((row) => {
        const qty = Number(row.querySelector(".item-qty").value) || 0;
        total += 15000 * qty;
      });
  ...
  async function fillRegularRow(row, item) {
    ...
    if (item.classKey) {
      ...
      unitSelect.innerHTML = buildOptionsHtml(
        units,
        "unitKey",
        "unitName",
        "규격 선택",
  ...
  document.getElementById("order-add")?.addEventListener("click", async () => {
    ...
    if (manualId) {
      ...
      document.getElementById("add-modal-title").textContent =
        `${activeCard.querySelector("strong").textContent} 수정`;
      ...
      if (list.length === 0) {
        ...
        list.forEach((item) => {
          ...
          row.querySelector(".item-approve-date").value =
            item.approveDate || "";
  ...
  document
    ...
    ?.addEventListener("click", async () => {
      if (addModalMode === "regular") {
        ...
        const items = Array.from(
          addRegularItemTableBody.querySelectorAll(".modal-item-row"),
        )
          .map((row) => ({
          ...
          .filter(
            (item) =>
              item.classKey && item.unitKey && item.qty && item.userCode,
          );

        if (items.length === 0) {
          if (
            !confirm(
              "입력된 품목이 없습니다. 기존 추가분을 모두 삭제하시겠습니까?",
            )
          )
            return;
        }
        ...
        const items = Array.from(
          addItemTableBody.querySelectorAll(".modal-item-row"),
        )
          .map((row) => ({
            orderDate: row.querySelector(".item-order-date").value,
            classKey: row.querySelector(".item-name").value.trim(),
        ...
        try {
          ...
          if (data.success) {
            ...
            const card = statementTabs.querySelector(
              `.statement-card[data-manual-id="${editedId}"]`,
            );
            if (card) {
              statementTabs
                .querySelectorAll(".statement-card")
  ...
  const updateStatementArrows = () => {
    const hasOverflow =
      statementTabs.scrollWidth > statementTabs.clientWidth + 1;
  ...
  const renderStatementCards = (manualList) => {
    statementTabs
      .querySelectorAll(".statement-card[data-manual-id]")
    ...
    if (deleteIcon) {
      ...
      try {
        ...
        if (data.success) {
          ...
          statementTabs
            .querySelectorAll(".statement-card")
            .forEach((c, idx) => c.classList.toggle("active", idx === 0));
          fetchInvoice();
          fetchManualList();
        } else {
          alert("삭제에 실패했습니다.");
  ...
  const refreshWaitCountBadge = () => {
    ...
    const total = Array.from(document.querySelectorAll(".center-item")).reduce(
      (sum, item) => sum + (parseInt(item.dataset.waitCount) || 0),
      0,
    );
  ...
  document
    .getElementById("deadlineSaveBtn")
    ?.addEventListener("click", async () => {
      const activeCenter = document.querySelector(".center-item.active");
      if (!activeCenter) {
        alert("먼저 센터를 선택해주세요.");
      ...
      const originalDay = activeCenter.dataset.deadline
        ? parseInt(activeCenter.dataset.deadline)
        : null;
      const centerCode = activeCenter.dataset.centerCode;
  ...
  const applyDeadlineToDisplay = (centerCode) => {
    const centerEl = document.querySelector(
      `.center-item[data-center-code="${centerCode}"]`,
    );
  ...
  const renderCenterInfo = (info) => {
    ...
    document.getElementById("infoBizNum").textContent = formatBizNum(
      info.bizNum,
    );
  ...
  const renderStatementTabs = (summaryList) => {
    ...
    firstCard.querySelector("small").textContent =
      `품목 ${summary.itemCount || 0}건 | 전체 ${summary.totalCount || 0}권`;
  ...
  const renderReorderTable = (list) => {
    ...
    list.forEach((item) => {
      ...
      const confirmedClass =
        item.confirmed === "checked"
          ? "approved"
          : item.confirmed === "unchecked"
            ? "wait"
            : item.confirmed === "user_cancel"
              ? "hold"
              : "rejected";
    ...
    tbody.querySelectorAll(".status-select").forEach((select) => {
      select.addEventListener("change", async () => {
        ...
        const activeCenter = document.querySelector(".center-item.active");
        const centerCode = activeCenter
          ? activeCenter.dataset.centerCode
          : null;
  ...
  const renderSummaryBox = (list) => {
    const orderTotal = list
      .filter((i) => i.rowType === "order")
      .reduce((s, i) => s + i.totalPrice, 0);
    const returnTotal = list
      .filter((i) => i.rowType === "return")
      .reduce((s, i) => s + Math.abs(i.totalPrice), 0);
    const addTotal = list
      .filter((i) => i.rowType === "add")
      .reduce((s, i) => s + i.totalPrice, 0);
    const finalTotal = orderTotal - returnTotal + addTotal;

    document.getElementById("summaryOrder").textContent =
      `${formatNumber(orderTotal)}원`;
    document.getElementById("summaryReturn").textContent =
      `${formatNumber(returnTotal)}원`;
    document.getElementById("summaryAdd").textContent =
      `${formatNumber(addTotal)}원`;
    document.getElementById("summaryTotal").textContent =
  ...
  const renderAggregateLoading = () => {
    ...
    ["summaryOrder", "summaryReturn", "summaryAdd", "summaryTotal"].forEach(
  ...
  const renderCenterAggregate = (list, segmentType) => {
    ...
    list.forEach((center) => {
      ...
      center.items.forEach((item, idx) => {
        if (isTeacherMode) {
          const isFirstOfTeacher =
            idx === 0 || center.items[idx - 1].userName !== item.userName;
          if (isFirstOfTeacher) {
        ...
        if (isTeacherMode) {
          const isLastOfTeacher =
            idx === center.items.length - 1 ||
            center.items[idx + 1].userName !== item.userName;
          if (isLastOfTeacher) {
            const sameTeacherItems = center.items.filter(
              (i) => i.userName === item.userName,
            );
            const tBase = sameTeacherItems.reduce((s, i) => s + i.baseCount, 0);
            const tTeacher = sameTeacherItems.reduce(
              (s, i) => s + i.teacherCount,
              0,
            );
            const tReorder = sameTeacherItems.reduce(
              (s, i) => s + i.reorderCount,
              0,
            );
            const tTotal = sameTeacherItems.reduce(
              (s, i) => s + i.totalCount,
              0,
            );
            const tTime = sameTeacherItems.reduce(
              (s, i) => s + i.timeTableCount,
              0,
            );
  ...
  document.querySelector(".primary-btn")?.addEventListener("click", () => {
    ...
    const orderedCenterCodes = Array.from(
  ...
  document
    .getElementById("onlyWaitCheck")
    .addEventListener("change", fetchReorderList);
  ...
  document.querySelector(".print-btn")?.addEventListener("click", () => {
    ...
    if (isAggregateMode) {
      ...
      printAggregate(
        year,
        month,
        lastAggregateSegment,
        lastAggregateCenterCodes,
      );
      return;
    }
  ...
  const orderTeacherListModal = document.getElementById(
    "order-teacher-list-modal",
  );
  const teacherSelect = document.getElementById("teacherSelect");
  const teacherSelectedName = document.getElementById("teacherSelectedName");
  const teacherHistoryTableBody = document.getElementById(
    "teacherHistoryTableBody",
  );
  ...
  document.querySelector(".list-btn")?.addEventListener("click", async () => {
    ...
    teacherSelect.innerHTML = `<option value="">불러오는 중...</option>`;
    teacherSelectedName.textContent = "선택된 선생님이 없습니다.";
    teacherHistoryTableBody.innerHTML = `<tr><td colspan="7" style="text-align:center; color:#9a9da7;">선생님을 선택해주세요.</td></tr>`;
    orderTeacherListModal?.classList.add("show");
    ...
    const options = (await optRes.json()).response;

    teacherSelect.innerHTML = buildOptionsHtml(
      options.users,
      "userCode",
      "userName",
      "선생님을 선택해주세요.",
  ...
  function renderTeacherHistoryByRound(list, deadlineDate) {
    if (!list || list.length === 0) {
      teacherHistoryTableBody.innerHTML = `<tr><td colspan="7" style="text-align:center; color:#9a9da7;">이번 달 주문 내역이 없습니다.</td></tr>`;
      return;
    }

    // round별로 그룹 (list는 round, className, unitName 순으로 이미 정렬되어 있음)
    const groups = [];
    list.forEach((item) => {
      const group = groups[groups.length - 1];
      if (!group || group.round !== item.round) {
        groups.push({ round: item.round, items: [item] });
      } else {
        group.items.push(item);
      }
    });

    // 마감일 이전 회차들은 전부 노이즈이므로, 마감 시점에 살아있던 마지막 회차 하나만 "기준 수량"으로 남기고 접는다.
    // 마감일이 없거나 모든 회차가 마감 이후라면 접지 않고 그대로 보여준다.
    let displayGroups = groups;
    let baselineRoundIndex = -1;

    if (deadlineDate) {
      for (let i = groups.length - 1; i >= 0; i--) {
        const firstItem = groups[i].items[0];
        const orderDate = firstItem.orderDate
          ? new Date(firstItem.orderDate.replace(" ", "T"))
          : null;
        if (orderDate && orderDate <= deadlineDate) {
          baselineRoundIndex = i;
          break;
        }
      }

      if (baselineRoundIndex > 0) {
        displayGroups = groups.slice(baselineRoundIndex);
      }
    }
    ...
    displayGroups.forEach((group, idx) => {
      const isBaseline = deadlineDate && idx === 0 && baselineRoundIndex >= 0;
      const isLast = idx === displayGroups.length - 1;

      const seq = idx + 1;
      let label;
      if (isBaseline) {
        label = `1회 · ${deadlineDate.getDate()}일 기준 수량`;
      } else if (isLast) {
        label = `${seq}회 · 최종`;
      } else {
        label = `${seq}회`;
      ...
      group.items.forEach((item) => {
        const key = `${item.classKey}|${item.unitKey}`;
        currentMap.set(key, {
          count: item.count || 0,
        ...
        if (prevMap === null) {
          ...
          remark = `<span class="diff-new">신규</span>`;
        } else {
          const diff = (item.count || 0) - prevMap.get(key).count;
          if (diff > 0) {
            remark = `<span class="diff-plus">+${diff}</span>`;
          } else if (diff < 0) {
            remark = `<span class="diff-minus">${diff}</span>`;
          } else {
            remark = "-";
          }
        }
      ...
      if (prevMap !== null) {
        prevMap.forEach((prevItem, key) => {
          ...
          rows.push(`
            <tr class="diff-removed-row">
              <td>-</td>
              <td>${prevItem.className || "-"}</td>
              <td>${prevItem.unitName || "-"}</td>
              <td>0</td>
              <td>-</td>
              <td>-</td>
              <td><span class="diff-removed">삭제</span></td>
            </tr>
          `);
        });
      }
    ...
    teacherHistoryTableBody.innerHTML = rows.join("");
    return displayGroups.length;
  }
  ...
  teacherSelect?.addEventListener("change", async () => {
    ...
    teacherSelectedName.textContent = `${selectedOption.textContent} 선생님`;
    teacherHistoryTableBody.innerHTML = `<tr><td colspan="7" style="text-align:center; color:#9a9da7;">불러오는 중...</td></tr>`;

    const res = await fetch("/logis/order/teacher-history", {
      method: "POST",
      headers: { "Content-Type": "application/json" },
      body: JSON.stringify({
        centerCode,
        userCode: teacherSelect.value,
        year,
        month,
      }),
    });
    ...
    // 시간표 등록 마감일: 모든 센터 공통 "전월 10일" (erp_order_deadline은 별개의 주문 마감일이라 여기서는 안 씀)
    const TIMETABLE_DEADLINE_DAY = 10;
    const deadlineDate = new Date(
      Number(year),
      Number(month) - 2,
      TIMETABLE_DEADLINE_DAY,
      23,
      59,
      59,
    );

    const displayCount = renderTeacherHistoryByRound(list, deadlineDate);
    teacherSelectedName.textContent = displayCount
      ? `${selectedOption.textContent} 선생님 · 총 ${displayCount}회`
      : `${selectedOption.textContent} 선생님 · 이번 달 주문 내역 없음`;
  });

  document
    .getElementById("teacher-list-modal-close-btn")
    ?.addEventListener("click", () => {
      orderTeacherListModal?.classList.remove("show");
    });
